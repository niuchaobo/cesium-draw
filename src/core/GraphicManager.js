...
class GraphicManager {
    ...
    set extrudedHeight(h) {
        this._extrudedHeight = h;
        if (this.editManager) {
            if (this.editManager.type === 'BOX') {
                this.editManager.options.polygon.extrudedHeight = this._extrudedHeight;
                this.editManager.graphic.polygon.extrudedHeight = this._extrudedHeight;
            }
        }
    }
    ...
    set material(v) {
        ...
        if (this.editManager) {
            ...
            } else if (this.editManager.type === 'POLYGON' || this.editManager.type === 'BOX') {
    ...
    set style(option) {
        ...
        if (this.editManager.type === 'POLYGON' || this.editManager.type === 'BOX') {
    ...
    createBox(options = CesiumBox.defaultStyle) {
        ...
        options.extrudedHeight = this.style.extrudedHeight || options.extrudedHeight;
        console.log(options)
    ...
    addEventListener() {
        ...
        const clickHandler = function (e) {
            ...
            if (self.mode === 'edit') {
                ...
                const topNodeGraphic = self.editManager.topNodeGrapchic
                ...
                if (pickedObj && known) {
                    if (pickedObj.id.mtype === GraphicType.POINT) {
                        ...
                        if (self.selectedNodeIndex !== -1) {
                            self.selectNodeType = 'bottom'
                            self.highlightedNode(pickedObj.id, nodeGraphic)
                        } else {
                            //再判断点击的是不是box的上部顶点
                            self.selectedNodeIndex = topNodeGraphic.contain(pickedObj.id)
                            if (self.selectedNodeIndex !== -1) {
                                self.selectNodeType = 'top'
                                self.highlightedNode(pickedObj.id, topNodeGraphic)
                            }
                        }
                    } else {
            ...
            if (self.graphicType != GraphicType.POLYLINE &&
                ...
                self.graphicType != GraphicType.BOX) {
                return;
            }
        ...
        const moseMoveHandler = function (e) {
            let cartesian = CVT.pixel2Cartesian2(e.endPosition, self.viewer);
            if (self.selectNodeType === "top") {
                let direction = e.endPosition.y - e.startPosition.y //负值向上，正值向下
                self.editManager.direction = direction
            }
            if (/.*MODEL.*/.test(self._heightReference)) {
                if (!viewer.scene.pickPositionSupported) {
                    console.log('This browser does not support pickPosition.')
    ...
    export(type) {
        ...
        let property = mapping[type];
        const managers = this.manager.values()
        for (let m of managers) {
            if (m.type === type) {
                console.log(m);
                let geoJson = m.toGeoJson()
                this.addProperty(m.options[property],geoJson);
                json.features.push(geoJson)
            }
        }
    ...
    import(feat) {
        ...
        if (feat.geometry.type.toUpperCase() === 'LineString'.toUpperCase()) {
        ...
        } else if (feat.geometry.type.toUpperCase() === "BOX") {
            coordinates = feat.geometry.coordinates[0]
            for (let c of coordinates) {
                positions.push({
                    lon: c[0],
            ...
            graphic = CesiumBox.fromDegrees(this.viewer, positions, feat.properties);
        } else {
    ...
    addProperty(mJson,geoJson){
        let properties = ['extrudedHeight','material','outline','outlineColor','outlineWidth','mcolor']
        properties.forEach(p=>{
            geoJson.properties[p] = mJson[p]
        })
    }
