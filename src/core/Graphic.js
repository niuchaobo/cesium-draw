...
class BaseGraphic {
    constructor(viewer) {
        ...
        this._attachment = []
        this._selectNodeType = "bottom";
        this.direction = undefined;
    }

    get selectNodeType() {
        return this._selectNodeType
    }

    set selectNodeType(v) {
        this._selectNodeType = v
    }
...
class CesiumBox extends BaseGraphic {
     ...
     * positions定义其位置信息， 属性信息遵循和Cesium.PolygonGraphic相同的定义方式
     */
    constructor(viewer, options = CesiumBox.defaultStyle) {
        super(viewer);
        this._type = 'BOX';
        ...
        this.graphic = undefined;
        this.nodeGraphic = undefined;
        this.topNodeGrapchic = undefined;
        this.outlineGraphic = undefined;
        this.outline = options.outline;
        this.create();
    }
    ...
    updateNode(index, node) {
        ...
        //因为多边形的最后一个点和第一个点是一样的（所以矩形的点是5个），在鼠标点选时不确定会选中哪个，
        // 可能会选中最后一个（即下标为4），而this.positions中的点为4个（为矩形时），所以会造成下标越界
        ...
        //判断选中的是下面的点还是上面的点，下面的点可以随意拖动，上面的点只能垂直方向拖动
        if (this.selectNodeType === "bottom") {
            this.positions[index] = node
            let t = this.getTopNodePosition(node);
            this.topNode[index] = t
            if (index === 0) {
                this.nodePositions[0] = node
                this.nodePositions[this.nodePositions.length - 1] = node
            } else {
            ...
            //计算在垂直方向实际移动的点位坐标
            let originNode = this.topNode[index]
            let cartographic = Cesium.Cartographic.fromCartesian(originNode);
            ...
            //原来每个点的高度都要重新计算
            let arr = []
            for (let n of this.topNode) {
                let cartographic = Cesium.Cartographic.fromCartesian(n);
                let lat = Cesium.Math.toDegrees(cartographic.latitude);
                let lng = Cesium.Math.toDegrees(cartographic.longitude);
                let tmp = Cesium.Cartesian3.fromDegrees(lng, lat, height);
                arr.push(tmp);
            }
            this.topNode = arr;
            this.topNodeGrapchic.positions = arr
            this.options.polygon.extrudedHeight = height;
            this.graphic.polygon.extrudedHeight = height;
        }
    }
    ...
    startEdit() {
        ...
        if(!this.topNodeFlag){
            ...
            for (let p of this.positions) {
                //arr.push(p);
                let t = this.getTopNodePosition(p);
                arr.push(t);
            }
            this.createTopNode(arr)
            this.topNode = arr;
        }
        if (this.graphic instanceof Cesium.Entity) {
            this.graphic.polygon.hierarchy = new Cesium.CallbackProperty(function () {
            ...
            if (this.outline) {
                ...
                this.createNode()
                //this.createBoxNode(arr)
            }
        }

    }

    getTopNodePosition(bottom) {
        let cartographic = Cesium.Cartographic.fromCartesian(bottom);
        let lat = Cesium.Math.toDegrees(cartographic.latitude);
        let lng = Cesium.Math.toDegrees(cartographic.longitude);
        let height = cartographic.height + this.graphic.polygon.extrudedHeight
        let top = Cesium.Cartesian3.fromDegrees(lng, lat, height);
        return top;
    }
     ...
     * 遵循和Cesiun.PointGraphic相同的定义方式
     */
    createTopNode(pos, options = CesiumPoint.defaultStyle) {
        this.topNodeFlag = true;
        options.positions = pos;
        options.clampToGround = this.graphic.polygon.perPositionHeight.getValue(this.viewer.clock.currentTime);
        this.topNodeGrapchic = new CesiumPoint(this.viewer, options);
    }
